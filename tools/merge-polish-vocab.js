#!/usr/bin/env node
/**
 * Merge Polish vocab explanations into data/vocab-hints.json + vocab-dictionary.json
 * Expects data/pl-batches/out-vocab-XX.json as { "term": "polish explanation", ... }
 * or [{ "term": "...", "pl": "..." }]
 */
const fs = require('fs');
...
function main() {
  ...
  for (const file of files) {
    ...
    if (Array.isArray(root)) {
      for (const item of root) {
        if (item?.term && item?.pl) plMap[item.term] = String(item.pl).trim();
      }
    } else {
      for (const [term, val] of Object.entries(root)) {
        if (typeof val === 'string') plMap[term] = val.trim();
        else if (val && typeof val.pl === 'string') plMap[term] = val.pl.trim();
      }
    }
  }
  ...
  for (const list of Object.values(hints)) {
    for (const h of list || []) {
      ...
      if (!pl && dict[h.term]?.translations?.pl) pl = dict[h.term].translations.pl;
      if (pl) {
        h.translations.pl = pl;
        appliedHints++;
      } else {
        missing++;
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        files: files.length,
        plMapSize: Object.keys(plMap).length,
        appliedHints,
        appliedDict,
