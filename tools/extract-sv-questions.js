...
// Extract all SV questions using a more flexible regex
const svQuestions = [];

// Match question blocks - handle both orders (promptEn before/after image)
const questionBlockRegex = /{\s*id:\s*"(SV-[^"]+)",\s*topic:\s*"safety-vehicle",[\s\S]*?options:\s*\[([^\]]+)\],/g;
...
while ((match = questionBlockRegex.exec(content)) !== null) {
  ...
  if (options.length > 0) {
    svQuestions.push({ id, promptEn, options });
  }
}

console.log(`Found ${svQuestions.length} safety-vehicle questions`);

if (svQuestions.length > 0) {
  console.log(`\nFirst 3 questions:`);
  svQuestions.slice(0, 3).forEach((q, idx) => {
...
svQuestions.forEach(q => {
  urTranslations["safety-vehicle"][q.id] = {
...
// Write to file
fs.writeFileSync('tools/sv-urdu-template.json', JSON.stringify(urTranslations, null, 2), 'utf8');
console.log(`\nTemplate saved to tools/sv-urdu-template.json`);
console.log(`Total questions: ${svQuestions.length}`);
