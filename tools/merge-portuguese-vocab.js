#!/usr/bin/env node
/**
 * Merge Portuguese vocab explanations into data/vocab-hints.json + vocab-dictionary.json
 * Expects data/pt-batches/out-vocab-XX.json as { "term": "portuguese explanation", ... }
 * or [{ "term": "...", "pt": "..." }]
 */
const fs = require('fs');
...
function main() {
  ...
  for (const file of files) {
    ...
    if (Array.isArray(root)) {
      for (const item of root) {
        if (item?.term && item?.pt) plMap[item.term] = String(item.pt).trim();
      }
    } else {
      for (const [term, val] of Object.entries(root)) {
        if (typeof val === 'string') plMap[term] = val.trim();
        else if (val && typeof val.pt === 'string') plMap[term] = val.pt.trim();
      }
    }
  }
  ...
  for (const list of Object.values(hints)) {
    for (const h of list || []) {
      ...
      if (!pl && dict[h.term]?.translations?.pt) pl = dict[h.term].translations.pt;
      if (pl) {
        h.translations.pt = pl;
        appliedHints++;
      } else {
        missing++;
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        files: files.length,
        ptMapSize: Object.keys(plMap).length,
        appliedHints,
        appliedDict,
