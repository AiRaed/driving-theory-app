#!/usr/bin/env node
/**
 * Merge Bengali question batch outputs into public/locales/bn.json (+ locales/bn.json)
 * Expects data/bn-batches/out-q-XX.json files shaped as:
 * { "TOPIC": { "ID": { "promptBn": "...", "options": [{ "bn": "..." }, ...] } } }
 * or flat: { "ID": { "topic", "promptBn", "options": [...] } }
 */
const fs = require('fs');
...
const OUT_PUBLIC = path.join(ROOT, 'public/locales/bn.json');
const OUT_LOCALES = path.join(ROOT, 'locales/bn.json');
...
function looksBengali(text) {
  if (!text || typeof text !== 'string') return false;
  const t = text.trim();
  if (t.length < 1) return false;
  // Reject Arabic/Urdu script contamination
...
function main() {
  ...
  for (const file of outFiles) {
    ...
    for (const [k, v] of Object.entries(root)) {
      if (k === 'meta') continue;
      if (v && typeof v === 'object' && (v.promptBn || v.options)) {
        // flat: id -> { topic?, promptBn, options }
        entries.push({ id: k, topic: v.topic || enMap[k]?.topic, ...v });
      } else if (v && typeof v === 'object') {
        // topic -> { id -> entry }
        for (const [id, entry] of Object.entries(v)) {
          if (entry && typeof entry === 'object' && entry.promptBn) {
    ...
    for (const entry of entries) {
      const { id, topic, promptBn, options } = entry;
      const en = enMap[id];
      if (!en) {
      ...
      if (!looksBengali(promptBn)) {
        errors.push({ id, reason: 'bad promptBn' });
        skipped++;
        continue;
      }
      if (promptBn.trim() === en.promptEn.trim()) {
      ...
      const plOpts = options.map((o) => (typeof o === 'string' ? o : o?.bn));
      if (
        plOpts.some((p, i) => {
          if (!looksBengali(p)) return true;
      ...
      if (!store[t]) store[t] = {};
      store[t][id] = {
        promptBn: promptBn.trim(),
        options: plOpts.map((pl) => ({ bn: String(pl).trim() })),
      };
      merged++;
    }
  }
