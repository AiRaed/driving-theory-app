#!/usr/bin/env node
/**
 * Merge Portuguese question batch outputs into public/locales/pt.json (+ locales/pt.json)
 * Expects data/pt-batches/out-q-XX.json files shaped as:
 * { "TOPIC": { "ID": { "promptPt": "...", "options": [{ "pt": "..." }, ...] } } }
 * or flat: { "ID": { "topic", "promptPt", "options": [...] } }
 */
const fs = require('fs');
const path = require('path');

const ROOT = path.join(__dirname, '..');
const BATCH_DIR = path.join(ROOT, 'data/pt-batches');
const OUT_PUBLIC = path.join(ROOT, 'public/locales/pt.json');
const OUT_LOCALES = path.join(ROOT, 'locales/pt.json');
...
function main() {
  ...
  for (const file of outFiles) {
    ...
    for (const [k, v] of Object.entries(root)) {
      if (k === 'meta') continue;
      if (v && typeof v === 'object' && (v.promptPt || v.options)) {
        // flat: id -> { topic?, promptPt, options }
        entries.push({ id: k, topic: v.topic || enMap[k]?.topic, ...v });
      } else if (v && typeof v === 'object') {
        // topic -> { id -> entry }
        for (const [id, entry] of Object.entries(v)) {
          if (entry && typeof entry === 'object' && entry.promptPt) {
    ...
    for (const entry of entries) {
      const { id, topic, promptPt, options } = entry;
      const en = enMap[id];
      if (!en) {
      ...
      if (!looksPortuguese(promptPt)) {
        errors.push({ id, reason: 'bad promptPt' });
        skipped++;
        continue;
      }
      if (promptPt.trim() === en.promptEn.trim()) {
      ...
      const plOpts = options.map((o) => (typeof o === 'string' ? o : o?.pt));
      if (
        plOpts.some((p, i) => {
          if (!looksPortuguese(p)) return true;
      ...
      if (!store[t]) store[t] = {};
      store[t][id] = {
        promptPt: promptPt.trim(),
        options: plOpts.map((pl) => ({ pt: String(pl).trim() })),
      };
      merged++;
    }
  }
